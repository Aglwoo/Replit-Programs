...
while run == True:
    ...
    if keys[pygame.K_DOWN]:
        y10 += vel
#left  
    if keys[pygame.K_a] and x > vel and left == True:
        height = heightcheck
        x -= vel
        pygame.display.update() 
#left supportchar
    if keys[pygame.K_LEFT] and x10 > vel and left == True:
        height = heightcheck
        x10 -= vel
        pygame.display.update() 
 
#right
    if keys[pygame.K_d] and x < 400 - vel - width:
        height = heightcheck
        x += vel
        pygame.display.update() 
#right supportchar
    if keys[pygame.K_RIGHT] and x10 < 400 - vel - width:
      height = heightcheck
      x10 += vel
      pygame.display.update() 
#jump        
    if not(isJump):
#jumping
        if keys[pygame.K_SPACE] and y < 380 - height - vel and y < 380 - height - vel or keys[pygame.K_w] and y < 380 - height - vel:
          height = heightcheck
          isJump = True
          pygame.display.update() 
# crouch                                          
        if not(keys[pygame.K_c]) and not(keys[pygame.K_s]):
            height = 70
            pygame.display.update()
            y = ycheck1
            pygame.display.update()
...
#hitbox
      ...
      charecter.stealthchar(x,y,left)
      if x >= 340:
        level = 2
        x = x1
        y= y1
...
#fifth level   
    if level == 5:
      ...
      if oy2 == 0 or oy2 == 1:
        ...
        oy2 = oy2 - 3
      oy3 = oy2
      if oy3 == 0 or oy3 == 1 or oy3 == 3 or oy3 == 2:
        yay = True             
      if yay == True:
        level = 6
#level 6
    if level == 6:
      ...
      charecter.stealthchar(x,y,left)
      charecter.supportchar(x10,y10)
      crate = pygame.draw.rect(win,(80,30,0),(cratex,cratey,70,70))
      button = pygame.draw.rect(win,(255,0,0),(100,320,20,20))
      base =pygame.draw.rect(win,(50,50,50),(95,340,30,5))
      doorx = pygame.draw.rect(win,(80,30,100),(250,270,70,70))
      door = "closed"
      if character.colliderect(crate) and x >= cratex:
        cratex = cratex-10
        pygame.display.update()
        print("MADE BY AGLWOO AND DARTHAJU FANX FOR PLAYIN INNIT")
